...
-- Aquí empieza mi resolución del ejercicio
USE universidad;


SELECT DISTINCT persona.apellido1, persona.apellido2, persona.nombre FROM persona  WHERE persona.tipo = 'alumno' ORDER BY persona.apellido1 ASC, persona.apellido2 ASC, persona.nombre ASC;
SELECT DISTINCT persona.apellido1, persona.apellido2, persona.nombre FROM persona  WHERE persona.tipo = 'alumno' AND (telefono IS NULL OR telefono = '');
SELECT DISTINCT persona.apellido1, persona.apellido2, persona.nombre FROM persona  WHERE persona.tipo = 'alumno' AND YEAR(persona.fecha_nacimiento) = 1999;
SELECT DISTINCT persona.apellido1, persona.apellido2, persona.nombre FROM persona  WHERE persona.tipo = 'profesor' AND (telefono IS NULL OR telefono = '') AND RIGHT(persona.nif, 1) = 'K' ;
...
SELECT a.nombre, c.anyo_inicio, c.anyo_fin FROM persona p JOIN alumno_se_matricula_asignatura al ON p.id = al.id_alumno JOIN curso_escolar c ON al.id_curso_escolar = c.id JOIN asignatura a ON a.id = al.id_asignatura WHERE p.nif = '26902806M';
SELECT DISTINCT departamento.nombre FROM departamento INNER JOIN profesor ON departamento.id = profesor.id_departamento INNER JOIN asignatura ON asignatura.id_profesor = profesor.id_profesor INNER JOIN grado ON grado.id = asignatura.id_grado WHERE grado.nombre = 'Grado en Ingeniería Informática (Plan 2015)';
SELECT DISTINCT persona.apellido1, persona.apellido2, persona.nombre FROM persona INNER JOIN alumno_se_matricula_asignatura ON persona.id = alumno_se_matricula_asignatura.id_alumno INNER JOIN curso_escolar ON alumno_se_matricula_asignatura.id_curso_escolar = curso_escolar.id WHERE persona.tipo = 'alumno' AND curso_escolar.anyo_inicio = '2018' ORDER BY persona.apellido1 ASC, persona.apellido2 ASC, persona.nombre ASC;
...
SELECT persona.apellido1, persona.apellido2, persona.nombre FROM persona LEFT JOIN profesor ON persona.id = profesor.id_profesor LEFT JOIN asignatura ON profesor.id_profesor = asignatura.id_profesor WHERE asignatura.id_profesor IS NULL AND persona.tipo = 'profesor' ORDER BY persona.apellido1 ASC, persona.apellido2 ASC, persona.nombre ASC;
...
SELECT departamento.id, departamento.nombre FROM departamento LEFT JOIN profesor ON departamento.id = profesor.id_departamento LEFT JOIN asignatura ON profesor.id_profesor = asignatura.id_profesor LEFT JOIN alumno_se_matricula_asignatura ON asignatura.id = alumno_se_matricula_asignatura.id_asignatura LEFT JOIN curso_escolar ON alumno_se_matricula_asignatura.id_curso_escolar = curso_escolar.id WHERE curso_escolar.id IS NULL GROUP BY departamento.id, departamento.nombre;

SELECT COUNT(*) FROM persona WHERE tipo='alumno';
SELECT COUNT(*) FROM persona WHERE tipo='alumno' AND YEAR(fecha_nacimiento) = 1999;
SELECT departamento.nombre, COUNT(persona.id) AS num_profesores FROM persona JOIN profesor ON persona.id = profesor.id_profesor JOIN departamento ON profesor.id_departamento = departamento.id WHERE persona.tipo = 'profesor' GROUP BY departamento.id HAVING num_profesores > 0 ORDER BY num_profesores DESC;
SELECT d.nombre AS departamento, COUNT(pr.id_profesor) AS num_profesores FROM departamento d LEFT JOIN profesor pr ON d.id = pr.id_departamento GROUP BY d.nombre ORDER BY num_profesores DESC, departamento ASC;
...
SELECT g.nombre, COUNT(*) as num_asignaturas FROM grado g LEFT JOIN asignatura a ON g.id = a.id_grado GROUP BY g.id HAVING num_asignaturas > 40 ORDER BY num_asignaturas DESC;
SELECT grado.nombre, asignatura.tipo, SUM(asignatura.creditos) AS creditos_totales FROM grado LEFT JOIN asignatura ON grado.id = asignatura.id_grado GROUP BY grado.nombre, asignatura.tipo ORDER BY grado.nombre ASC, creditos_totales DESC;
SELECT curso_escolar.anyo_inicio, COUNT(DISTINCT ama.id_alumno) AS num_alumnos FROM curso_escolar LEFT JOIN alumno_se_matricula_asignatura ama ON curso_escolar.id = ama.id_curso_escolar LEFT JOIN asignatura ON asignatura.id = ama.id_asignatura GROUP BY curso_escolar.anyo_inicio ORDER BY curso_escolar.anyo_inicio ASC;
SELECT p.id, p.nombre, p.apellido1, p.apellido2, COUNT(a.id) AS num_asignaturas FROM persona p LEFT JOIN asignatura a ON p.id = a.id_profesor WHERE p.tipo = 'profesor' GROUP BY p.id, p.nombre, p.apellido1, p.apellido2 ORDER BY num_asignaturas DESC;
SELECT * FROM persona WHERE tipo = 'alumno' AND fecha_nacimiento = ( SELECT MAX(fecha_nacimiento) FROM persona WHERE tipo = 'alumno');
SELECT p.id, p.nombre, p.apellido1, p.apellido2 FROM persona p JOIN profesor pr ON p.id = pr.id_profesor JOIN departamento d ON pr.id_departamento = d.id WHERE NOT EXISTS (SELECT 1 FROM asignatura a WHERE a.id_profesor = p.id) ORDER BY p.apellido1 ASC, p.apellido2 ASC, p.nombre ASC;
